...
function renderTimerLabel(remaining) {
  const quizMeta = elements.quizArea.querySelector('.quiz-meta.top');
  const markup = `
  ...
  if (quizMeta) {
    ...
    elements.quizArea.insertAdjacentHTML('afterend', markup);
  }
}
...
function handleAnswer(button, question) {
  ...
  const explanationBox = elements.quizArea.querySelector('.explanation-box');
  const nextBtn = document.getElementById('nextQuestionBtn');

  buttons.forEach((item) => {
    item.disabled = true;
    if (Number(item.dataset.choice) === question.answer) {
      item.classList.add('correct');
    }
  });

  const userChoice = Number(button.dataset.choice);
  button.classList.add(userChoice === question.answer ? 'correct' : 'wrong');
      ...
      <div class="mistake-list">
        ${state.quiz.questions.map((question, index) => `
          <div class="mistake-item">
            <p><strong>Soal ${index + 1}</strong> - ${question.topicName}</p>
            <p>${question.prompt}</p>
